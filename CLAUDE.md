...
- 🐛 **Mismo patrón, también corregido el 2026-07-19**: `calcularBolsasAcumuladas` (bolsas.ts) y el `resumenGuardado` de `/cierre/general` (page.tsx) tampoco pasaban el % congelado. **Pendiente, NO corregido a propósito**: el "Resumen del turno" dentro de `CierreGeneralForm.tsx` (vista previa en vivo, antes de guardar) sigue sin leer el % configurado. Es cosmético (lo guardado sí usa el % correcto), pero si se activa Tercero o se cambia el % global, esa vista previa mostrará números viejos hasta refrescar.
