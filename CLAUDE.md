...
- 🐛 **Bug encontrado y corregido el 2026-07-19**: `calcularBolsasAcumuladas` (bolsas.ts) y el `resumenGuardado` de `/cierre/general` (page.tsx) reconstruían el input a `calcularCierreGeneral` **sin pasar el % congelado del cierre** — siempre usaban el default (70/0), sin importar el % real con el que se guardó cada cierre. Ya corregido en ambos sitios. **Pendiente, NO corregido a propósito**: el "Resumen del turno" dentro de `CierreGeneralForm.tsx` (vista previa en vivo en Movimientos, antes de guardar) tiene el mismo problema — no lee el % configurado. Es cosmético (lo que se guarda de verdad sí usa el % correcto), pero si algún día se activa Tercero o se cambia el % global, esa vista previa mostrará números viejos hasta refrescar.
