...
import { crearClienteLicor } from "../actions/cartera";
import { registrarVentaLicor } from "../actions/ventas";
import {
  LICOR_MEDIOS_PAGO,
...
// Pop-up de venta de cerveza. Se abre desde el botón "Venta Licores Jhoann" que la vendedora
// ya conoce, para no cambiarle la pantalla (pedido del dueño, 2026-07-19). Guarda la venta en
// el módulo Licores SIEMPRE, y solo con Nequi/Efectivo alimenta además el cuadre Nequi.
export interface ClienteOpcion {
  id: string;
  nombre: string;
}

export function VentaLicorModal({
  productos,
  clientes,
  shift,
  onClose,
  ...
  function elegirProducto(id: string) {
    ...
    setError(null);
    // El precio se autocompleta al escoger el producto (pedido del dueño).
    const p = productos.find((x) => x.id === id);
    setPrecioUnitario(p ? p.precioVenta : null);
  }

  function guardar() {
    if (!producto) return setError("Elige la cerveza");
    ...
    if (metodoPago === "CREDITO" && !clienteId)
      return setError("Elige a quién le estás fiando");

    setError(null);
    startTransition(async () => {
      const r = await registrarVentaLicor({
      ...
      <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-white p-4 shadow-xl sm:rounded-2xl">
        ...
        {productos.length === 0 ? (
          <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-700">
            Todavía no hay cervezas registradas. El administrador debe crearlas primero.
          </p>
        ) : (
          <>
            <div className="mb-3">
              ...
              <>
                {sinStock && (
                  <p className="mb-3 rounded-lg bg-red-50 p-2 text-sm font-medium text-red-700">
                    {producto.nombre} está agotada — no se puede vender.
                  </p>
                )}
                ...
                {/* Aviso de descuento: el precio salió de la lista, así que si lo cambia hay
                    que preguntarle si es a propósito (pedido del dueño). */}
                {hayDescuento && (
                ...
                <div className="mb-3">
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    {metodoPago === "CREDITO"
                      ? "Queda como fiado: la cerveza sale del inventario y la plata queda por cobrar."
                      ...
                      <div className="mt-2 flex gap-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setError(null);
                            startTransition(async () => {
                              const r = await crearClienteLicor({ nombre });
                              if (r.ok) {
                                setClienteNuevo("");
                                setCreandoCliente(false);
                                // El servidor ya lo guardó; refrescamos para que aparezca
                                // en el selector con su id real.
                                router.refresh();
