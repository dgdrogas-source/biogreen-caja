...
import { crearCategoriaGasto, eliminarCategoriaGasto } from "../actions/categoriasGasto";
...
// Categorías de gasto editables por el admin (Servicios públicos, Nómina, etc.). Añadir es
// libre; eliminar desactiva en vez de borrar si la categoría ya tiene gastos registrados
// (el propio backend lo decide y avisa).
export function CategoriasGastoConfig({ items }: { items: CategoriaGastoItem[] }) {
  ...
  function agregar() {
    ...
    startTransition(async () => {
      const r = await crearCategoriaGasto(nombre.trim());
      if (r.ok) {
        setNombre("");
        router.refresh();
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm">
      ...
      <p className="mb-4 text-xs text-gray-500">
        Se usan al registrar un gasto en el Cierre general. Puedes añadir o eliminar las que
        necesites.
      </p>
