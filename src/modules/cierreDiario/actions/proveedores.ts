...
// "Eliminar" desactiva si el proveedor ya tiene facturas/gastos asociados en algún parte de
// turno (protege el histórico); se borra físicamente solo si nunca se usó.
export async function eliminarProveedor(id: string): Promise<ActionResult> {
  try {
    ...
    const [enFacturas, enGastos] = await Promise.all([
      prisma.parteTurnoFactura.count({ where: { proveedorId: id } }),
      prisma.parteTurnoGasto.count({ where: { proveedorId: id } }),
    ]);
