...
// "Eliminar" desactiva si la categoría ya tiene gastos asociados en algún parte de turno
// (protege el histórico); se borra físicamente solo si nunca se usó.
export async function eliminarCategoriaGasto(id: string): Promise<ActionResult> {
  try {
    ...
    const enUso = await prisma.parteTurnoGasto.count({ where: { categoriaId: id } });
