...
import { afectaCuadreNequi, FUXION_MEDIOS_PAGO, type ActionResult } from "../types";
...
const ventaSchema = z.object({
  ...
  metodoPago: z.enum(FUXION_MEDIOS_PAGO),
  shift: z.union([z.literal(1), z.literal(2)]),
  nota: z.string().trim().max(300).optional(),
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Fecha inválida").optional(), // solo admin
  // Obligatorio solo si se fía: hay que saber a quién cobrarle (cartera propia de Fuxion).
  clienteId: z.string().optional(),
});

export type VentaFuxionInput = z.infer<typeof ventaSchema>;
...
async function estadoActual(productoId: string) {
  const [producto, compras, ventas] = await Promise.all([
    prisma.fuxionProducto.findUnique({
      where: { id: productoId },
      select: { inventarioInicial: true },
    }),
...
// Registra una venta de Fuxion (vendedora o admin), desde el pop-up de "Venta Fuxion".
// Si se cobró por Nequi o Efectivo, crea ADEMÁS el ingreso en el cuadre Nequi — una sola vez.
// Con tarjeta/Daviplata/transferencia/crédito la plata no pasa por la caja Nequi: solo Fuxion.
export async function registrarVentaFuxion(input: VentaFuxionInput): Promise<ActionResult> {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const movementId = businessDayId
        ? await crearMovementLigado(tx, {
            ...
            // Igual que el registro manual de hoy: una venta de Fuxion alimenta su bolsillo.
            pettyCashBucket: "FUXION",
            note: `${data.cantidad} × ${producto.nombre}`,
            userId: user.id,
          })
        : null;

      const venta = await tx.fuxionVenta.create({
        data: {
...
// Borra una venta y su ingreso ligado en el cuadre Nequi. Permisos (mismo patrón que los
// movimientos de Nequi y que Licores): el admin borra cualquiera, cualquier día; la vendedora
// solo las suyas y solo del día de hoy.
export async function eliminarVentaFuxion(id: string): Promise<ActionResult> {
  try {
    const user = await requireUser();

    const venta = await prisma.fuxionVenta.findUnique({
      where: { id },
      include: { producto: { select: { nombre: true } } },
    });
    if (!venta || venta.deletedAt) return { ok: false, error: "Venta no encontrada" };
    if (venta.fuxionCierreId)
      return {
        ok: false,
        error: "Esa venta ya entró en un cierre de Fuxion. Deshaz el cierre para poder borrarla.",
      };
    ...
    await prisma.$transaction(async (tx) => {
      await tx.fuxionVenta.update({ where: { id }, data: { deletedAt: new Date() } });
