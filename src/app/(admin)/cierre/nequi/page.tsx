...
import { getBaseFund, getCurrentShift, getDaySummary, getDiscrepancies, getPockets } from "@/modules/nequi/queries";
import { MOVEMENT_LABELS, POCKET_BUCKETS, SHIFT_LABELS, type MovementType, type Shift } from "@/modules/nequi/types";

const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

export default async function CierreNequiPage({
  searchParams,
}: {
  searchParams: Promise<{ fecha?: string; turno?: string; dDesde?: string; dHasta?: string; dTurno?: string }>;
}) {
  await requireAdmin();
  ...
  const dTurno: Shift | undefined = params.dTurno === "1" ? 1 : params.dTurno === "2" ? 2 : undefined;
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-lg font-bold capitalize text-gray-800">Cierre Nequi — {formatDateCo(date)}</h1>
        <div className="flex flex-wrap items-center gap-2">
          <TurnoTabs date={date} shift={shift} basePath="/cierre/nequi" />
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm">
        <h2 className="mb-3 text-base font-semibold text-gray-800">Resumen por categoría — {SHIFT_LABELS[shift]}</h2>
        {rows.length === 0 ? (
            ...
            {rows.map(([type, t]) => (
              <div key={type} className="flex items-center justify-between py-2 text-sm">
                ...
                <span className="text-right">
                  <span className="font-semibold text-gray-800">${t.nequi.toLocaleString("es-CO")}</span>
                  {t.efectivo > 0 && (
                    <span className="block text-xs text-amber-600">+ ${t.efectivo.toLocaleString("es-CO")} efectivo</span>
                  )}
                </span>
              </div>
            ))}
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <form className="flex flex-wrap items-center gap-2">
            ...
            <button className="rounded-lg bg-gray-800 px-3 py-1.5 text-xs font-medium text-white">Filtrar</button>
          </form>
        </div>
        ...
        {discrepancies.length === 0 ? (
          ...
          <div className="overflow-x-auto">
                  ...
                  return (
                    <tr key={d.id}>
                      ...
                      <td className="whitespace-nowrap px-2 py-2 text-right text-gray-600">
                        {d.closingExpectedBalance !== null ? `$${d.closingExpectedBalance.toLocaleString("es-CO")}` : "—"}
                      </td>
                      <td className="whitespace-nowrap px-2 py-2 text-right text-gray-600">
                        {d.closingRealBalance !== null ? `$${d.closingRealBalance.toLocaleString("es-CO")}` : "—"}
                      </td>
            ...
            <p className="mt-2 text-xs text-gray-400">
              El siguiente turno siempre arranca con el saldo REAL del cierre anterior: el descuadre queda
              registrado aquí y no se arrastra al cálculo.
            </p>
          </div>
        )}
      </div>
